...
U računanju hipervolumena sam koristila funkciju hv iz biblioteke pygmo.
...
Maksimalni raspon sam računala po formuli:
\begin{equation}
MS = \sqrt{ \sum_{k=1}^{r} \left( f_k^{\max} - f_k^{\min} \right) }
\end{equation}
...
Rezultati maksimalnog raspona pokazuju da su vrednosti mog NSGA-II algoritma malo veće u poređenju sa rezultatima pymoo implementacije.
Ovo sugeriše da su rešenja generisana mojim algoritmom koncentrisanija u malo širem delu Pareto fronta, što znači da postiže malo bolju pokrivenost u širini za razliku od pymoo implementacije.
...
Primećuju se skoro iste razlike kao kod prethodnog poređenja, samo što su još manje razlike kod metrike maksimalnog raspona. SPEA2 u biblioteci pymoo je implementiran tako da daje iste rezultate kao i NSGA-II iz iste biblioteke.
...
Drugi faktor je način na koji se mutacija i ukrštanje sprovode. Obe implementacije koriste polinomijalnu mutaciju, samo što pymoo ima prilagođene distribucione parametre koji omogućavaju bolje balansiranje intenzifikacije i diverzifikacije rešenja, dok moja implementacija koristi fiksne parametre.  Kao rezultat, pymoo može generisati rešenja koja su ravnomernije raspoređena po frontu, dok moja verzija pokazuje malo veći maksimalni raspon (MS) u određenim situacijama, što znači da obuhvata malo širu oblast ciljnog prostora. Veći MS u mojoj implementaciji ukazuje da algoritam uspeva da pronađe ekstremne tačke Pareto fronta, ali može dovesti do manje ravnomerne raspodele rešenja, pri čemu neka područja fronta mogu ostati slabije pokrivena.
